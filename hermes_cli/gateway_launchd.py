...
def _launchctl_bootstrap(domain: str, plist_path, label: str, *, timeout: int = 30) -> None:
    ...
    try:
        ...
        _wait_for_launchd_label_unloaded(domain, label, timeout=_launchd_reload_budget())
        subprocess.run(bootstrap, check=True, timeout=timeout)
...
def _wait_for_launchd_label_unloaded(domain: str, label: str, *, timeout: float) -> bool:
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            _append_launchd_reload_log(f"{domain}/{label} still loaded {timeout:.0f}s after bootout")
            return False
        time.sleep(0.5)
...
def _spawn_deferred_launchd_reload(
    *, domain: str, label: str, target: str, plist_path: Path, gateway_pid: int
    ...
    # (mirrors _launchd_print_service_pid).
